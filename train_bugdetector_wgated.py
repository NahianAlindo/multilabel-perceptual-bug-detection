...
def main():
    ...
    tr_idx, va_idx, te_idx = stratified_splits(
        all_records, train_ratio=args.train_ratio, val_ratio=args.val_ratio,
        seed=args.seed, min_combo_support=5
    )
    ...
    print(f"Split -> train: {len(train_recs)} | val: {len(val_recs)} | test: {len(te_idx)} (test not used here)")
