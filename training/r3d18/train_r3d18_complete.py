#!/usr/bin/env python3
# ==============================================================================
# R3D-18 MODEL - Complete Training + Validation + Test Pipeline
# ==============================================================================
# 3D ResNet-18 for video classification
# Purpose: Compare 3D ResNet vs I3D and 2D+temporal approaches
...
class BugClipDataset(Dataset):
    def __init__(self, data_root: Path, records: list, split: str = "train",
                 num_frames: int = 16, resize_hw: int = 112):  # R3D uses 112x112
...
class BasicStem(nn.Sequential):
    """R3D stem: 3D conv with spatial and temporal downsampling."""
    def __init__(self):
        super().__init__(
...
class R3D18(nn.Module):
    ...
    def __init__(self, num_classes=400, dropout=0.5):
        super().__init__()
        ...
        # Residual blocks
        self.layer1 = self._make_layer(BasicBlock3D, Conv3DSimple, 64, 2, stride=1)
        self.layer2 = self._make_layer(BasicBlock3D, Conv3DSimple, 128, 2, stride=2)
        self.layer3 = self._make_layer(BasicBlock3D, Conv3DSimple, 256, 2, stride=2)
        self.layer4 = self._make_layer(BasicBlock3D, Conv3DSimple, 512, 2, stride=2)
...
class BugDetectorR3D18(nn.Module):
    """R3D-18 model for multilabel bug detection."""
    def __init__(self, num_bug_types=5, dropout=0.5):
        super().__init__()
        
        # R3D-18 backbone (output 512 features)
        self.r3d = R3D18(num_classes=512, dropout=dropout)
        
        # Prediction heads
        self.head_types = nn.Linear(512, num_bug_types)
        self.head_count = nn.Linear(512, 4)  # 0, 1, 2, 3+
        self.head_any = nn.Linear(512, 1)
    
    def forward(self, x):
        # x: [B, T, C, H, W]
        features = self.r3d(x)  # [B, 512]
...
def create_test_visualizations(test_results, per_class_metrics, per_count_metrics,
    ...
    ax.set_ylabel('Score')
    ax.set_title('Test Set Performance: Per-Class Bug Type Metrics (R3D-18 Model)')
    ax.set_xticks(x)
    ...
    ax.set_ylabel('Score')
    ax.set_title('Test Set Performance: Per-Count Class Metrics (R3D-18 Model)')
    ax.set_xticks(x)
    ...
    ax.set_title('Test Set: Bug Count Confusion Matrix (R3D-18 Model)')
    plt.tight_layout()
    ...
    if len(combo_metrics) > 0:
        ...
        ax.set_ylabel('Score')
        ax.set_title('Test Set: Top 20 Bug Combination Performance (R3D-18 Model)')
        ax.set_xticks(x)
...
def main():
    ...
    parser.add_argument('--batch-size', type=int, default=8)  # R3D is memory-intensive
    ...
    parser.add_argument('--wandb-project', type=str, default='r3d18-bug-detection')
    ...
    print(f"\n{'='*80}")
    print("R3D-18 MODEL - BUG DETECTION TRAINING")
    print(f"{'='*80}\n")
    ...
    # Datasets (R3D uses 112x112 resolution)
    ds_train = BugClipDataset(Path(args.data_root), train_recs, "train", 16, 112)
    ds_val = BugClipDataset(Path(args.data_root), val_recs, "val", 16, 112)
    ...
    # Model
    model = BugDetectorR3D18(num_bug_types=len(CANON_BUG_TYPES), dropout=0.5).to(device)
    print(f"✓ Model: R3D-18 on {device}")
    ...
    if len(test_recs) > 0:
        ...
        ds_test = BugClipDataset(Path(args.data_root), test_recs, "test", 16, 112)
        ...
        test_probs_types = test_preds_types.astype(float)
