#!/usr/bin/env python3
# ==============================================================================
# ResNet18-ONLY MODEL - Complete Training + Validation + Test Pipeline
# ==============================================================================
# Temporal ablation baseline: ResNet18 with temporal pooling (NO LSTM/GRU)
# Purpose: Prove that temporal modeling (BiLSTM/GRU) is necessary
...
# ==============================================================================
# MODEL ARCHITECTURE - ResNet18-ONLY (No Temporal Modeling)
# ==============================================================================

class ResNet18OnlyModel(nn.Module):
    """ResNet18 with temporal pooling (NO LSTM/GRU) - Temporal Ablation Baseline."""
    def __init__(self, num_bug_types=5, dropout=0.5, pretrained=True, pool_type='avg'):
        super().__init__()
        ...
        # Prediction heads (directly from pooled features)
    ...
    def forward(self, frames):
        ...
        # Temporal pooling (NO LSTM/GRU - this is the ablation!)
        if self.pool_type == 'avg':
            pooled_feats = feats.mean(dim=1)  # [B, 512] - simple average
        elif self.pool_type == 'max':
            pooled_feats = feats.max(dim=1)[0]  # [B, 512] - max pooling
        else:
            pooled_feats = feats[:, -1, :]  # [B, 512] - last frame only
        
        pooled_feats = self.dropout(pooled_feats)
        
        # Predictions
        logits_types = self.head_types(pooled_feats)
        logits_count = self.head_count(pooled_feats)
        logits_any = self.head_any(pooled_feats).squeeze(-1)
...
def create_test_visualizations(test_results, per_class_metrics, per_count_metrics,
    ...
    ax.set_ylabel('Score')
    ax.set_title('Test Set Performance: Per-Class Bug Type Metrics (ResNet18-Only Model)')
    ax.set_xticks(x)
    ...
    ax.set_ylabel('Score')
    ax.set_title('Test Set Performance: Per-Count Class Metrics (ResNet18-Only Model)')
    ax.set_xticks(x)
    ...
    ax.set_title('Test Set: Bug Count Confusion Matrix (ResNet18-Only Model)')
    plt.tight_layout()
    ...
    if len(combo_metrics) > 0:
        ...
        ax.set_ylabel('Score')
        ax.set_title('Test Set: Top 20 Bug Combination Performance (ResNet18-Only Model)')
        ax.set_xticks(x)
...
def main():
    ...
    parser.add_argument('--pool-type', type=str, default='avg', choices=['avg', 'max', 'last'])
    parser.add_argument('--wandb-project', type=str, default='resnet18-only-bug-detection')
    ...
    print(f"\n{'='*80}")
    print("ResNet18-ONLY MODEL - BUG DETECTION TRAINING")
    print("(Temporal Ablation Baseline - No LSTM/GRU)")
    print(f"{'='*80}\n")
    ...
    # Model
    model = ResNet18OnlyModel(num_bug_types=len(CANON_BUG_TYPES), dropout=0.5, pool_type=args.pool_type).to(device)
    print(f"✓ Model: ResNet18-Only ({args.pool_type} pooling) on {device}")
