...
#SBATCH --job-name=bugdet_r18only
#SBATCH --output=/home/nahian26/projects/def-loutfouz/nahian26/bugdatasetbig/logs_r18only/training_%j.out
#SBATCH --error=/home/nahian26/projects/def-loutfouz/nahian26/bugdatasetbig/logs_r18only/training_%j.err
#SBATCH --mail-user=nahian.rifaat@ontariotechu.net
#SBATCH --mail-type=BEGIN,END,FAIL,TIME_LIMIT_80

# ==============================================================================
# ResNet18-ONLY MODEL - Bug Detection Training (Temporal Ablation Baseline)
# For: nahian26@narval.alliancecan.ca
...
#
# PURPOSE: Temporal Ablation - Prove LSTM/GRU helps vs frame averaging
# ==============================================================================

echo "=============================================="
echo "ResNet18-ONLY MODEL - Bug Detection Training"
echo "(Temporal Ablation: No LSTM/GRU)"
echo "=============================================="
...
# ==============================================================================
# Paths (ResNet18-only specific directories)
# ==============================================================================

BASE_DIR="/home/nahian26/projects/def-loutfouz/nahian26/bugdatasetbig"
DATA_ROOT="$BASE_DIR"
MANIFEST="$BASE_DIR/my_new_splits.jsonl"
SCRIPT="$BASE_DIR/scripts_r18only/train_resnet18_only_complete.py"
CHECKPOINT_DIR="/home/nahian26/scratch/checkpoints/resnet18_only"
TENSORBOARD_DIR="/home/nahian26/scratch/runs/resnet18_only"
LOGS_DIR="$BASE_DIR/logs_r18only"
...
export WANDB_PROJECT="resnet18-only-bug-detection"
...
if [ ! -f "$SCRIPT" ]; then
    ...
    echo "   Please upload train_resnet18_only_complete.py to scripts_r18only/ directory"
    exit 1
fi
...
echo ""
echo "🚀 Starting ResNet18-Only training..."
echo "📊 Monitor at: https://wandb.ai (if enabled)"
echo "   TensorBoard: tensorboard --logdir=$TENSORBOARD_DIR"
echo ""
echo "Configuration:"
echo "  Model: ResNet18-Only (NO temporal modeling)"
echo "  Temporal: Frame averaging (no LSTM/GRU)"
...
echo "  Test F1:     ~78% (baseline without temporal)"
echo "  Comparison:  vs BiLSTM ~87% = -9% without temporal!"
echo "  Purpose:     Prove temporal modeling (LSTM/GRU) helps"
echo ""
...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    echo "   # Best model"
    echo "   scp nahian26@narval:$BEST_CKPT ./"
    echo ""
    echo "   # Test results"
    echo "   scp nahian26@narval:$TEST_RESULTS ./"
    echo ""
    echo "   # All visualizations"
    echo "   scp -r nahian26@narval:$VIZ_DIR ./"
    echo ""
    ...
    echo "🔄 To continue:"
    echo "   cd $BASE_DIR"
    echo "   sbatch run_resnet18_only_training.sh"
    echo ""
    ...
    echo "   - Path issues → Verify manifest and clips directory"
    echo ""
fi
